...
public class UserController {
	...
	/**
	 * 用户注册接口
	 * 接收JSON数据，包含username，passwordHash，email，name字段
	 * 传入的passwordHash此时为明文密码
	 */
	@RequestMapping(value="/api/user/register",method=RequestMethod.POST)
	@ResponseBody
	public ResponseModel register(@RequestBody User user){
		//User user=UserService.fromJSON(userJSONString);
		if (user==null) {
	...
	public ResponseModel authenticate(
		...
		//尝试获取授权Token
		Token token=UserService.authenticate(username, password);
		//授权失败
		if (token==null) {
			ErrorResponse response=new ErrorResponse();
	...
	/**
	 * 文章上传接口
	 * 接收JSON字符串格式数据，其中包含两个对象：{Token,Article}
	 * 范例如下
	...
	@ResponseBody
	public ResponseModel uploadAriticle(@RequestBody UploadArticleModel uploadArticleModel){
	...
	/**
	 * 点赞接口，包含Token和LikeId两个对象。接收JSON字符串。
	 * JSON范例如下：
	 * {"token":{"userId":1,"token":"25309d38-1adf-419e-9481-9b34e57cabba",
	 * "tokenDeadline":1461393807828},"likeId":{"userUserId":1,"articleArticleId":24}}
	 * @param likeModel
	 * @return
	 */
	...
	public ResponseModel like(@RequestBody LikeModel likeModel){
		...
		if (!UserService.like(likeModel.getLikeId().getUserUserId(),
				likeModel.getLikeId().getArticleArticleId())) {
	...
	public ResponseModel bookmark(@RequestBody BookmarkModel bookmarkModel){
		 ...
		 if (!UserService.bookmark(token.getUserId(), bookmarkModel.getArticleId())) {
